...
def hls_transform(img, s_thresh, sx_thresh, sgrad_thresh, kernel_size):
    img = np.copy(img)
    ...
    # Sobel y
    sobely = cv2.Sobel(l_channel, cv2.CV_64F, 0, 1, ksize=kernel_size)  # Take the derivative in y
    scaled_sobel_y = normalize(sobely)
    absgraddir = np.arctan2(np.absolute(sobely), np.absolute(sobelx))
    s_grad_dir =  np.zeros_like(absgraddir)
    s_grad_dir[(absgraddir >= sgrad_thresh[0]) & (absgraddir <= sgrad_thresh[1])] = 1

    # Threshold color channel
    s_binary = np.zeros_like(s_channel)
    s_binary[(s_channel >= s_thresh[0]) & (s_channel <= s_thresh[1])
             & (absgraddir >= sgrad_thresh[0]) & (absgraddir <= sgrad_thresh[1])] = 1

    return (sxbinary, s_binary)

# Convert to single channel binary image
# Uses HLS color transformation and gradient sobel
def custom_gray_transform(img, s_thresh=(170, 255), sx_thresh=(20, 100), sgrad_thresh=(30 * np.pi/180, 70 * np.pi/180), kernel_size =3):
    sxbinary, s_binary = hls_transform(img, s_thresh, sx_thresh, sgrad_thresh, kernel_size)

    out_img = np.zeros_like(sxbinary)
    out_img[(sxbinary == 1) | (s_binary == 1)] = 1
    return out_img

def get_hls_transform_image(img, s_thresh=(170, 255), sx_thresh=(20, 100), sgrad_thresh=(0, np.pi/2), kernel_size =3):
    sxbinary, s_binary = hls_transform(img, s_thresh, sx_thresh, sgrad_thresh, kernel_size)
